...
int getOPTAB(FILE *fp4,struct optabcode oplist[])
{
    ...
    while(!feof(fp4))
    {
        ...
        if(temp=='\n')
        {
            ...
            oplist[j] = tokenizeOpcode(line);
            //oplist[j] = temp_optab;

            //printf("%s %X\n",oplist[j].opcode,oplist[j].objectcode);
...
int main()
{
    ...
    struct optabcode oplist[6];
    int optable_size;
    optable_size = getOPTAB(fp4,oplist);
