...
void addSymbolTable(int *address, struct instruction inst)
{
...
*/
int ProcessFirstLine(FILE *fp1,FILE *fp2,struct instruction temp_inst,int *address)
{
    char temp;
    char line[30];
    int i=0;
    ...
    while(!feof(fp1)){
        ...
        if(temp=='\n')
        {
            ...
            if(start_f==1){
                ...
                if(temp_inst.label!="")
                {
                    addSymbolTable(address,temp_inst);
                }
...
int ProcessLines(FILE *fp1,FILE *fp2,struct instruction temp_inst,int *address)
{
    char temp;  // Temporary character variable
    char line[30];  // Temporary string to hold a line/instruction
    int i=0;
...
int main()
{
    ...
    linecount = ProcessFirstLine(fp1,fp2,temp_inst,&address);

    // Read Remaining Lines
    linecount += ProcessLines(fp1,fp2,temp_inst,&address);
