...
void getObjectCode(struct instruction temp_inst,char opcodearray[][MAXOP],int codearray[],char symbolarray[][MAXOP],int addarray[],char hexop[])
{
...
void ProcessTextLines(FILE *fp1,FILE *fp2,FILE *fp3,FILE *fp4,FILE *fp5,char opcodearray[][MAXOP],int codearray[],char symbolarray[][MAXOP],int addarray[])
{
    ...
    while(!feof(fp1))
    {
       ...
       strcpy(operandhere,temp_inst.operand);
        ...
        hexop[0]='\0';
        getObjectCode(temp_inst,opcodearray,codearray,symbolarray,addarray,hexop) ;
...
int main()
{
    ...
    printf("\n\n");
    int optab_size = getOPTAB(fp3,opcodearray,codearray);
    ...
    printf("\n\n");
    int symtab_size = getSYMTAB(fp2,symbolarray,addarray);
