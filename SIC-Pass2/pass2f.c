...
/*
void InitializeTextRecord(FILE *fp4,int starting_location)
{
    fprintf(fp4,"T^%7d",starting_location);
}
*/
...
int getSYMADD(char symbol[],char symbolarray[][MAXOP],int addarray[])
{
    ...
    /*
    if(strstr(symbol,",X")!=NULL||strstr(symbol,",x")!=NULL)
    {
...
void ProcessTextLines(FILE *fp1,FILE *fp2,FILE *fp3,FILE *fp4,FILE *fp5,char opcodearray[][MAXOP],int codearray[],char symbolarray[][MAXOP],int addarray[])
{
    ...
    int textrecordflag = 0;

    struct stringarray symbols[MAXOP];

    int firstentry = 1;
    int starting_address;
    char firstcode[MAXOP];
    firstcode[0]='\0';
    ...
    while(!feof(fp1))
    {
        ...
        hexop[0]='\0';
        int inst_size = getObjectCode(temp_inst,opcodearray,codearray,symbolarray,addarray,hexop);

        sizecounter = sizecounter+inst_size;

        strcpy(symbols[i].hexcode,hexop);

        if(firstentry==1)
        {
            starting_address=temp_inst.location;
            firstentry=0;
        }

        if(sizecounter>30||feof(fp1))
        {
            fprintf(fp4,"T^%X^%X",(sizecounter-inst_size),starting_address);
            for(int j=0;j<i;j++)
            {
                fprintf(fp4,"^%s",symbols[i].hexcode);
            }
            fprintf(fp4,"\n");

            strcpy(symbols[0].hexcode,symbols[i].hexcode);
            starting_address=temp_inst.location;
